...
| 环节                       | 类型                   | 说明                    |
| ------------------------ | -------------------- | --------------------- |
| GMEM（输入 A、B）             | **BF16**             | 原始输入矩阵就是 bf16 存的      |
| TMA load → SMEM（输入 tile） | **BF16**             | TMA 只搬字节，不做任何转换       |
| Tensor Core 读输入          | **BF16**             | mma 从 smem 按描述符读 bf16 |
| Tensor Core 内部           | **BF16 乘 → FP32 累加** | 转换点①，硬件自动完成           |
| TMEM（累加器 D）              | **FP32**             | 每个元素占 4B              |
| `tcgen05.ld` → 寄存器       | **FP32**             | 原样读出                  |
| 寄存器里 cvt 之后              | **BF16**             | 转换点②，软件显式转换           |
